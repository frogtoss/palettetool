/* ftg_core.h - Frogtoss Toolbox.  Public domain-like license below.
   ...
   ftg libraries are copyright (C) 2015-2024 Frogtoss Games, Inc.
...
#define FTG__STATIC_INITIALIZE_VALUE(P, SZ, N) memset((P), 0xFE, (SZ) * (N));
#else
...
#define FTG_STATIC_ARRAY_NAMED_DECL(T, NAME, N)  \
    typedef struct array_##NAME##_##T {          \
        T ptr[(N)];                              \
        size_t count;                           \
        FTG__CAPACITY_FIELD                     \
    } array_##NAME##_##T;
...
ftg__arena_grow(ftg_arena_t** arena, size_t min_size)
{
    ...
    a->ptr = a->start = (uint8_t*)FTG_MALLOC(size, 1);
    a->end = a->ptr + size;
}
